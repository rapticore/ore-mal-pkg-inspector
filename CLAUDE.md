# OreWatch — Claude Code Integration Guide

This repository uses the `orewatch` MCP server to enforce supply-chain safety
before any dependency is introduced. Claude Code **must** follow the rules in
this file whenever it adds or updates a dependency.
...
| MCP configured | MCP block in Claude Code settings (see below) |
...
Copy the printed JSON block into your Claude Code MCP configuration
(`.claude/settings.json` or the IDE MCP settings panel):
...
> **Tip:** `orewatch monitor ide-bootstrap --client claude_code` regenerates
> this block at any time. It resolves the absolute path to the local binary
...
Once connected, Claude Code has access to six OreWatch tools:
...
|---|---|---|---|
| `client_type` | string | yes | Always `"claude_code"` |
...
```json
{
  ...
  "results": [
    { "name": "chalk", "status": "clean", "severity": "none" }
  ],
...
| Field | Type | Required |
|---|---|---|
| `client_type` | string | yes — `"claude_code"` |
...
1. **Call `orewatch_check_dependency_add`** with `client_type: "claude_code"`,
...
| Symptom | Fix |
|---|---|
| MCP tools not visible | Re-run `orewatch monitor ide-bootstrap --client claude_code` and update your MCP config |
